...
public class YaclibPackagePlugin extends DefaultTask {
    ...
    public Boolean packageApp() throws IOException, InterruptedException {
        ...
        final String debianPackageLocation = FileProcessUtilities.INSTANCE.getActualLocation(InitUtilities.DpkgDeb);
        ...
        new RunBuilder(appName, CommonStringsHelper.INSTANCE.buildProjectRunScriptLocation(appName)).build();
        new RunBuilder(appName, CommonStringsHelper.INSTANCE.buildSystemRunScriptLocation(appName)).build();

        new StopBuilder(serverPort, CommonStringsHelper.INSTANCE.buildProjectStopScriptLocation(appName)).build();
        new StopBuilder(serverPort, CommonStringsHelper.INSTANCE.buildSystemStopScriptLocation(appName)).build();

        new InitDBuilder(appName, serverPort, CommonStringsHelper.INSTANCE.buildDebianEtcPath(appName)).build();

        new DebianPackageBuilder(appName, serverVersion, maintainerInfo, CommonStringsHelper.DebianBuildBuild + "DEBIAN/control")
                .build();
        ...
        FileUtils.copyDirectory(
                new File(CommonStringsHelper.BuildInstallPath + appName),
                new File(CommonStringsHelper.DebianBuildBuild + "opt/" + appName));

        final File debianDirectory =  new File(CommonStringsHelper.DebianBuild + appName + "_" + serverVersion + "_all");
        ...
        final YaclibModel.ProcessReport report = FileProcessUtilities.INSTANCE.executeProcess(currentDirectory, InitUtilities.DpkgDeb, "--build " + debianDirectory);
